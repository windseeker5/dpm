...
def session_attendance(args, language):
    ...
    answer = (
        f"J’ai trouvé {len(records)} présence(s) enregistrée(s) pour {activity_label(activity, language)}."
        if language == "fr"
        else f"I found {len(records)} recorded attendance entries for {activity_label(activity, language)}."
    )
    columns = (
