...
Fields:
  ...
  title        short plain name shown in the dashboard's test list (optional; derived from script name if missing)
  summary      one plain line shown collapsed in the dashboard; click it to read `description` and `verifies`
...
CATALOG = [
    dict(
        order="00", script="00_preflight.py", area="Preflight", title="Log in to demo", summary="Can we log in to the demo site, on desktop and on a phone?",
    ...
    dict(
        order="01", script="01_activity_with_photo.py", area="Activity lifecycle", title="Create Wing Foil activity with photo", summary="Create a Wing Foil course with a cover photo, then edit it and archive it.",
    ...
    dict(
        order="01b", script="01b_activity_no_photo.py", area="Activity lifecycle", title="Create activity without photo", summary="Create an activity with no photo: the default image must show, never a broken one.",
    ...
    dict(
        order="01c", script="01c_activity_surf_ai_photo_location.py", area="Activity lifecycle", title="Create Surf activity (photo search + location)", summary="Create a Surf course using the photo search and the address lookup (both depend on outside services).",
    ...
    dict(
        order="02", script="02_signup_payment_first.py", area="Signup", title="Sign up, payment first", summary="A customer signs up and pays first: the signup must wait for payment.",
    ...
    dict(
        order="02b", script="02b_signup_approval_first.py", area="Signup", title="Sign up, approval first", summary="A customer signs up, then the admin approves the signup.",
    ...
    dict(
        order="03", script="03_passport_admin_create.py", area="Passport", title="Admin creates a passport", summary="The admin gives a passport straight to a customer, without a signup.",
    ...
    dict(
        order="03b", script="03b_passport_inheritance.py", area="Passport", title="Passport inherited by a second activity", summary="A passport from one activity must also work on a second activity.",
    ...
    dict(
        order="04", script="04_checkin_qr.py", area="Check-in / QR", title="Check in a passport (QR)", summary="Check a customer in at the door with their pass code and see one session used up.",
    ...
    dict(
        order="05", script="05_announcements_discord.py", area="Announcements & Discord", title="Send an announcement", summary="Send an announcement to a customer by email and by Discord.",
    ...
    dict(
        order="06", script="06_shop_nonmoney.py", area="Shop (non-money paths)", title="Shop: browse and cart (no payment)", summary="Browse the shop and fill a cart with a passport and two products, then stop before paying.",
    ...
    dict(
        order="07", script="07_surveys.py", area="Surveys", title="Send and answer a survey", summary="Send a survey, answer it on desktop and phone, then see and export the results.",
    ...
    dict(
        order="08", script="08_data_export.py", area="Data export", title="Export data", summary="Download every export (signups, finances, contacts, passports, surveys) and check none is empty.",
    ...
    dict(
        order="09", script="09_activity_log_crosscheck.py", area="Activity Log cross-check", title="Activity Log cross-check", summary="One last look at the Activity Log: everything this run did should be listed there.",
    ...
    dict(
        order="10", script="10_outbound_doc_links.py", area="Outbound doc links", title="Links in outgoing emails and documents", summary="Check that every documentation link in the app opens.",
    ...
    dict(
        order="11", script="11_wayne_ai.py", area="Wayne AI", title="Wayne AI assistant", summary="Ask the Wayne assistant its usual questions and check it answers.",
    ...
    dict(
        order="12", script="12_late_payment_reminders.py", area="Late payment reminders", title="Late payment reminders", summary="Trigger the late-payment reminder and check it is logged.",
    ...
    dict(
        order="13a", script="13a_branding_b1.py", area="Branding & fallback matrix", title="Branding check B1", summary="Signup, approval, passport and emails with an organization logo, an activity logo and a cover picture.",
    ...
    dict(
        order="13b", script="13b_branding_b2.py", area="Branding & fallback matrix", title="Branding check B2", summary="Same flow, without a cover picture.",
    ...
    dict(
        order="13c", script="13c_branding_b3.py", area="Branding & fallback matrix", title="Branding check B3", summary="Same flow, without an activity logo.",
    ...
    dict(
        order="13d", script="13d_branding_b4.py", area="Branding & fallback matrix", title="Branding check B4", summary="Same flow, with only the organization logo.",
    ...
    dict(
        order="13e", script="13e_branding_b5.py", area="Branding & fallback matrix", title="Branding check B5", summary="Same flow, without an organization logo.",
    ...
    dict(
        order="13f", script="13f_branding_b6.py", area="Branding & fallback matrix", title="Branding check B6", summary="Same flow, with only an activity logo.",
    ...
    dict(
        order="13g", script="13g_branding_b7.py", area="Branding & fallback matrix", title="Branding check B7", summary="Same flow, with only a cover picture.",
    ...
    dict(
        order="13h", script="13h_branding_b8.py", area="Branding & fallback matrix", title="Branding check B8", summary="Same flow, with no logo and no picture at all (full fallback).",
    ...
    dict(
        order="90", script="90_production_stripe_live.py", area="[REAL MONEY] Stripe payment", title="Pay $1 with Stripe (real money)", summary="Pay for real with Stripe. You type your own card; the test only reads the total.",
    ...
    dict(
        order="92", script=None, area="[MANUAL] Backup / restore", title="Backup and restore (by hand)", summary="Backup and restore, by hand only (a restore overwrites the real database).",
